...
function mixinDiscovery(DB2, db2) {
  ...
  DB2.prototype.paginateSQL = function(sql, orderBy, options) {
    ...
    if (options.offset || options.skip || options.limit) {
      ...
      const offset = Number(options.offset || options.skip || 0);
      if (isNaN(offset)) {
        offset = 0;
      }
      if (options.limit) {
  ...
  DB2.prototype.buildQueryPrimaryKeys = function(schema, table) {
    debug('DB2.prototype.buildQueryPrimaryKeys: schema=%j, table=%j',
      schema, table);
    const sql = 'SELECT TRIM(tabschema) AS "owner",' +
      ' TRIM(tabname) AS "tableName",' +
      ' TRIM(colname) AS "columnName",' +
      ' colseq AS "keySeq",' +
      ' constname AS "pkName"' +
  ...
  DB2.prototype.buildQueryExportedForeignKeys = function(schema, table) {
    debug('DB2.prototype.buildQueryExportedForeignKeys: schema=%j, table=%j',
      schema, table);
    const sql = 'SELECT TRIM(a.constraint_name) AS "fkName",' +
